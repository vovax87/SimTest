...
@interface PerformanceAreaStep () <AVAudioPlayerDelegate>
...
-(NSDictionary *)saveSettings{
    ...
    NSNumber *index1 = [NSNumber numberWithInt:_selectedIndexes[1]];
    NSNumber *index2 = [NSNumber numberWithInt:_selectedIndexes[0]];
...
-(void)selectSoundByMovingType:(NSString*)keyMovingType
{
    ...
    if (dirName.length)
    {
        ...
        if (list.count) {
            ...
            // run delegate method
            if ([self.delegate respondsToSelector:@selector(performanceArea:didSelectSoundURL:)]) {
                [self.delegate performanceArea:self didSelectSoundURL:selectedSoundURL];
            }
        }
    }
}
...
- (IBAction)actionMovingTypeTapped:(UITapGestureRecognizer *)sender
{
    NSUInteger ind = sender.view.tag;
