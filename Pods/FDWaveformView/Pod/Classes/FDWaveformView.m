...
- (void)initialize
{
    ...
    self.wavesColor = [UIColor blueColor];
...
- (void)setAudioURL:(NSURL *)audioURL
{
    ...
    NSError *error = nil;
    self.player = [[AVAudioPlayer alloc] initWithContentsOfURL:_audioURL error:&error];
}
...
-(void)drawBorder{
    ...
    self.layer.borderColor = [UIColor colorWithRed:0 green:195.f/255.f blue:1 alpha:1].CGColor;
...
{
    FDWaveformView *copy = [[[self class] alloc] init];
    
    copy.frame = self.frame;
    copy.audioURL = self.audioURL;
   
    return copy;
}

-(void)dealloc{
    NSLog(@"deallo FDWaveView");
}
...
-(void)playSound{
    ...
    if (self.timeToStartPlay > 0) {
        
        AVAudioPlayer* __weak weakPlayer = self.player;
        
        
//        self.player.volume = 0.1;
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(self.timeToStartPlay * NSEC_PER_SEC)), audio_queue, ^{
